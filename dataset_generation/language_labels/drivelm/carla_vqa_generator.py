...
class QAsGenerator():
    ...
    def create_qa_pairs(self, path_id):
        ...
        try:
            ...
            with open(f"{self.output_directory}/failed_paths.txt", "a") as f:
        ...
        if self.save_examples:
            ...
            path_orgimg = f'{self.output_directory}/original_images/{scenario_name}/{route_folder}/{route_file_number}_{route_number}'
            Path(path_orgimg).mkdir(parents=True, exist_ok=True)
            path_resimg = f'{self.output_directory}/resized_images/{scenario_name}/{route_folder}/{route_file_number}_{route_number}'
            Path(path_resimg).mkdir(parents=True, exist_ok=True)
            path_graph = f'{self.output_directory}/graphs/{scenario_name}/{route_folder}/{route_file_number}_{route_number}'
            Path(path_graph).mkdir(parents=True, exist_ok=True)
            ...
            # Save QA data
            file_name = f'{path_graph}/{frame_number}.json'
            with open(file_name, 'w', encoding='utf-8') as f:
                json.dump(qas, f, sort_keys=True, indent=4)

            # Save org image
            image.save(f'{path_orgimg}/{frame_number}.jpg')

            # cut bottom part of image according to min_y and max_y
        ...
        self.convert_carla_to_nuscenes_and_save(relative_image_path, vqa_llava_entry)
    ...
    def save_stats(self):
        ...
        # Save statistics
        with open(os.path.join(self.output_directory, 'stats.json'), 'w', encoding="utf-8") as f:
            json.dump({
    ...
    def convert_carla_to_nuscenes_and_save(self, relative_image_path, vqa_llava_entry):
        """
        Convert a CARLA format file to a Nuscenes format file.
        """
        # Generate a random scene ID
        scene_id = self.generate_random_string()

        # Generate a random tick ID
        tick_id = self.generate_random_string()
        tick_data = {}

        # Store key object information
        tick_data['key_object_infos'] = vqa_llava_entry['conversations']['key_object_infos']
        ...
        # Store image path
        image_path = vqa_llava_entry['image']
        ...
        for key, items in vqa_llava_entry['conversations'].items():
        ...
        # easier to debug:
        save_dir = (self.output_directory + "/" + relative_image_path.split("/data/")[1]).replace('rgb', 'vqa').replace('.jpg', '.json.gz')
        # final version
        # save_dir = path.replace('/rgb/', '/drivelm_vqa/').replace('.jpg', '.json')
        
        # save_dir = os.path.join(args.output_directory, scenario_name, route_number)
        ...
        def determine_braking_requirement(qas_conversation_ego, pedestrians, measurements, vehicles, ego_vehicle, 
            ...
            if measurements['control_brake'] or measurements['speed'] < 0.9 * 0.72 * measurements['speed_limit'] \
                    ...
                    if 'AccidentTwoWays' in scenario_type \
                        ...
                        for vehicle in vehicles_by_id.values():
                            ...
                            if not consider_vehicle:
                                continue
                            keep_vehicle = is_vehicle_visible_in_image(vehicle, self.MIN_X, self.MAX_X, self.MIN_Y, self.MAX_Y, self.CAMERA_MATRIX)
                            ...
                            if vehicle['base_type'] == 'bicycle' and 'scenario' in vehicle['role_name'] and \
                                            ((ego_data['distance_to_junction'] is not None and ego_data['distance_to_junction'] < 10) or ego_data['is_in_junction']) and \
                        ...
                        consider_vehicle = should_consider_vehicle(actor_hazard, self.MIN_X, self.MAX_X, self.MIN_Y, self.MAX_Y, self.CAMERA_MATRIX)
                ...
                if leading_vehicle_id is not None:
                    ...
                    consider_vehicle = should_consider_vehicle(leading_vehicle, self.MIN_X, self.MAX_X, self.MIN_Y, self.MAX_Y, self.CAMERA_MATRIX)
                    if consider_vehicle:
            ...
            if answer == "There is no reason for the ego vehicle to brake." and measurements['control_brake']:
                if scenario_name == 'Accident':
                    ...
                    if bicycles:
                        # assert len(bicycles) == 2, f"Num bycicles is {len(bicycles)} but should be 2."
                        ...
                        if closest_bicycle['distance'] < 40:
                            answer = f"The ego vehicle should {brake_or_stop} because it must change the lane " \
                                        f"to bypass the {len(bicycles)} bicycles."
            ...
            if 'ConstructionObstacle' in scenario_name:
                ...
                if ego_data['lane_type_str'] == 'Parking':
                    relevant_objects = [x for x in vehicles_by_id.values() if 'scenario' in x['role_name'] 
                                        ...
                                        and x['position'][0]>0 and x['position'][0]<20] # added smaller than 20 in case of multiple scenarios (sometime the enxt scenario is already spawned)
                    if len(relevant_objects) > 1:
                        breakpoint()
                    assert len(relevant_objects) == 1, f'Expected 1 vehicle, found {len(relevant_objects)}, path: {self.current_path}'
            ...
            if scenario_name in ['Accident', 'AccidentTwoWays', 'ConstructionObstacle', 'ConstructionObstacleTwoWays',
                ...
                if 'HazardAtSideLane' in scenario_name:
                    relevant_objects = [v for v in vehicles_by_id.values() if v['base_type'] == 'bicycle' 
                                                                        and should_consider_vehicle(v, self.MIN_X, self.MAX_X, self.MIN_Y, self.MAX_Y, self.CAMERA_MATRIX)
    ...
    def generate_vehicle_information(self, other_vehicles, ego_vehicle, important_objects, key_object_infos, 
        ...
        def determine_path_crossing(current_measurement, ego_vehicle, other_vehicle_location_description, 
            ...
            if command_distance > 25:
                command_int = 4
            # ADDED: sometimes command is a lane change but we don't change the lane and the TP also indicates just going straight
            if (command_int == 5 or command_int == 6) and abs(current_measurement['target_point'][1]) < 1.0:
                if ego_distance_to_junction is not None and ego_distance_to_junction < 40:
                    command_int = 3
                else:
                    command_int = 4
        ...
        def determine_other_vehicle_position(other_vehicle_location_description, other_vehicle, ego_vehicle,
            ...
            if other_vehicle['is_in_junction'] and (other_vehicle['junction_id'] == ego_vehicle['next_junction_id'] or \
                ...
                if scenario == "MergerIntoSlowTrafficV2": # or scenario == "MergerIntoSlowTraffic":
                    if ego_vehicle['distance_to_junction'] is None:
                    ...
                    if ego_vehicle['num_lanes_same_direction'] == 1 and other_vehicle['same_road_as_ego'] or \
                        ...
                        answer = f"The {other_vehicle_description} is on the exit lane of the highway."
                                        ...
                                        (ego_vehicle['is_in_junction'] or ego_vehicle['distance_to_junction'] is not None and ego_vehicle['distance_to_junction'] < 25)):
        ...
        for vehicle in other_vehicles:
            ...
            if -2 <= vehicle['position'][1] <= 2:
                rough_pos_str = f'{far_or_close}to the front of the ego vehicle'
            elif vehicle['position'][1] > 2:
                rough_pos_str = f'{far_or_close}to the front right of the ego vehicle'
            else: #vehicle['position'][1] < -2
    ...
    def analyze_road_layout(self, ego_vehicle_info, important_objects, key_object_infos, current_measurement, scenario):
        ...
        def analyze_lane_marking(ego_vehicle_info, qas_conversation_roadlayout):
            ...
            for side_key, side_name in zip(keys, names):
                ...
                # https://carla.org/Doxygen/html/d6/dd3/structcarla_1_1road_1_1element_1_1LaneMarking.html
                ...
                elif ego_vehicle_info[f'{side_key}_marking_type'] == 4:
                    lanetype = "solid broken"
                elif ego_vehicle_info[f'{side_key}_marking_type'] == 5:
                    lanetype = "broken solid"
                elif ego_vehicle_info[f'{side_key}_marking_type'] == 6:
                    lanetype = "double broken"
        ...
        def analyze_important_lanes(command_description, command_int, lane_change_soon, is_junction, ego_vehicle_info, 
            ...
            if is_acceleration_lane and command_int==5:
                ...
                # when merging from the left side on the highway
                answer = f"The ego vehicle should pay particular attention to traffic on the leftmost lane of the " +\
                                f"highway, adjust its speed, and position itself so that no vehicle is driving on " +\
                                f"the same height to prepare for the lane change."
